...
```{r}
model1<-lm( date ~ Light + Humidity + Temperature + Occupancy + HumidityRatio,data=rawDF)
summary(model1)
```
...
```{r}
trainDF_feat <- cleanDF[1:471,  ]
testDF_feat <- cleanDF[471:843,  ]
trainDF_labels <- cleanDF[1:471,  2]
testDF_labels <- cleanDF[471:843,  2]
...
text and code here
```{r}
confusionMatrix(cleanDF_occupancy_pred, testDF_labels[[2]], positive = NULL, dnn = c("Prediction", "True"))
```
